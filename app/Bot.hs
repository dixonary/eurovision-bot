...
runBot = do
  ...
  err <- liftIO $ runDiscord $ def {
    discordToken   = "Bot NzEyMzM4ODEyNTU0MTgyNjY2.XsQLpA.jvTX5BeUzsDhVpq90R7I7dz8M_Q",
  ...
  if
    ...
    -- Assume it's a score update and act appropriately
        case parseScore messageText of
          Left err -> call_ dh $ Req.CreateMessage
              messageChannel (T.pack err)
          Right score -> do
...
startVote msg dh = do
  ...
  case dmChannelE of
    ...
    messagesE <- call dh $ Req.GetChannelMessages 
      (channelId dmChannel) 
    ...
    ballotMsgE <- call dh $ Req.CreateMessage 
      (channelId dmChannel)
      $ showBallot mempty

    modifyState (\s@State{..} -> s {
    ...
    case ballotMsgE of
      ...
      Right msg -> do
        modifyState (\s@State{..} -> s {
          scoreMessages = Map.insert uid (messageId msg) scoreMessages
          })
        return ()
...
call :: (Req.Request (r a), FromJSON a)
     => DiscordHandle -> r a -> Eurovision (Either RestCallErrorCode a)
call dh req = liftIO $ restCall dh req
  

-- Call an API endpoint, discarding the result.
call_ :: (Req.Request (r a), FromJSON a)
      => DiscordHandle -> r a -> Eurovision ()
